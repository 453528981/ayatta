...
MediaScope varchar(800) not null default '' comment '活动限定媒体Id 空为无限定 如需限定部分媒体 使用","分隔',
Global bool not null default 0 comment '适用于(全场店铺)所有商品',
Discount bool not null default 0 comment 'true为满件折 false为满元减', 
LimitBy tinyint not null default 0 comment '用户参与活动限制', 
LimitValue int not null default 0 comment '用户参与活动限制值 Limit为true时有效', 
...
primary key (Id)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='店铺促销';
...
Upon decimal(8,2) not null default 0 comment '临界值', 
Value decimal(8,2) not null default 0 comment '满减/满折值', 
SendGift bool not null default 0 comment '送赠品', 
GiftJson nvarchar(2000) not null default '' comment '赠品信息 Json格式', 
SendCoupon bool not null default 0 comment '送店铺优惠券', 
CouponJson nvarchar(2000) not null default '' comment '优惠券信息 Json格式', 
SellerId int not null default 0 comment '卖家Id', 
...
ModifiedOn timestamp not null default current_timestamp on update current_timestamp comment '最后一次编辑时间',
primary key (Id)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='店铺促销规则';
...
LimitBy tinyint not null default 0 comment '用户参与活动限制 0为无限制 1为LimitValue次 2为每用户LimitValue次',
LimitValue int not null default 0 comment '用户参与活动限制值 Limit为true时有效',
SellerId int not null default 0 comment '卖家Id', 
Status bool not null default 0 comment '状态 1为可用 0为不可用',
CreatedOn datetime not null default current_timestamp comment '创建时间',
...
Category tinyint not null default 0 comment 'A打折  B减价  C促销价 活动创建后,优惠方式将不能修改',
StartedOn datetime not null comment '开始时间',
StoppedOn datetime not null comment '结束时间',
Plateform tinyint not null default 0 comment '活动适用平台 0为None 1为适用平于pc 2为适用平于wap 4为适用平于app',
MediaScope varchar(800) not null default '' comment '限定媒体Id 空为无限定 如需限定部分媒体 使用","分隔',
...
LimitBy tinyint not null default 0 comment '用户参与活动限制 0为无限制 1为LimitValue次 2为每用户LimitValue次',
LimitValue int not null default 0 comment '用户参与活动限制值 Limit为true时有效',
SkuJson nvarchar(2000) not null default '' comment '对Sku设置的优惠信息 Json格式',
SellerId int not null default 0 comment '卖家Id', 
Status bool not null default 0 comment '状态 1为可用 0为不可用',
CreatedOn datetime not null default current_timestamp comment '创建时间',
ModifiedBy nvarchar(50) not null default '' comment '最后一次编辑者',
...
StartedOn datetime not null comment '开始时间',
StoppedOn datetime not null comment '结束时间',
Discount bool not null default 0 comment '1为打折 0为减元',
